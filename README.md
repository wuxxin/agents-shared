...
|---------------|-----------------|------------------------|
| **[Local Chat/Vision](#local-chat-services)** | [20080](http://localhost:20080) | Llama-server serving Chat/Vision LLM (embeddings disabled) |
| **[Local Embedding](#local-embedding-services)** | [20082](http://localhost:20082) | TEI (`text-embeddings-router`) or `llama-server` serving Text Embeddings |
| **[Local Reranking](#local-reranking-services)** | [20086](http://localhost:20086) | Llama-server serving Document Reranking |
| **[Local Speech to Text](#local-speech-to-text)** | [20090](http://localhost:20090) | Whisper-server audio transcription API (HTTP) |
| **[Local Text to Speech](#local-text-to-speech)** | [20095](http://localhost:20095) | Qwen3-tts-server audio synthesis API (HTTP) |
| **[Local Image Generation](#local-image-services)** | [20100](http://localhost:20100) | sd-server serving Image Generation API (HTTP) |
| **[Local Memory Service](#local-memory-service)** | [28888](http://localhost:28888), [28889](http://localhost:28889), [8890](http://localhost:8890) | Hindsight API (28888), Worker Control Plane (28889) & Control UI (8890) |
| **[Local Router](#local-combined-inference-router)** | [21080](http://localhost:21080) | Combined service router / OpenAI proxy (HTTP) |
| **[Local Inference Coordinator](#local-inference-coordinator)** | - | Combined service control script |

### Integration Services
| Service | Default Port(s) | Description / Protocol |
|---------------|-----------------|------------------------|
| **[Signal Integration](#signal-integration)** | [20889](http://localhost:20889), [20888](http://localhost:20888), 20887 |REST-API:20889, HTTP/JSON-RPC: 20888, TCP/JSON-RPC:20887 |
...
| **[IronClaw](#ironclaw)** | Rust | [28080](http://localhost:28080) | IronClaw Web Gateway & HTTP Webhooks |
...
- **Features**: Chat completions (`20080`) service
...
- **Sandboxing**: Requires `PrivateDevices=no` to access `/dev/dri` and `/dev/kfd`. Enforces `ProtectSystem=strict` while bind-mounting the user's home configuration and granting read-write access to `/data/public/machine-learning`.
- **Features**: Standalone text embedding server (`20082`) with mean pooling, ROCm GPU offloading, dynamic batching, and systemd sitecustomize helper patch.
...
- **Features**: Rerank Service (`20086`) services.
- Documentation:  [local-rerank.md](assistants/local-rerank.md)

### Local Speech-to-Text
- **Description**: Manages a persistent `whisper-server` instance for speech-to-text (STT) transcription. Serves an OpenAI-compatible audio transcription API on port 20090.
- **Sandboxing**: Requires `PrivateDevices=no` to access `/dev/dri` and `/dev/kfd` for GPU-accelerated synthesis (unless run in cpu only mode). Enforces `ProtectSystem=strict` while restricting filesystem access to the home directory and read-only system files.
- **Features**: audio transcoding using `ffmpeg`.
- Documentation: [local-speech-to-text.md](assistants/local-speech-to-text.md)

### Local Text-to-Speech
- **Description**: Manages a persistent `qwen3-tts-server` instance for text-to-speech (TTS) synthesis. Serves an OpenAI-compatible audio synthesis API on port 20095.
...
- **Description**: Manages a persistent `sd-server` instance for image generation. Serves an OpenAI-compatible image generation API on port 20100.
...
- **Description**: Manages a persistent Hindsight memory instance served by `hindsight-api` (port 28888) with a dedicated background task worker sidecar `hindsight-worker` serving a control plane / metrics HTTP server on port 28889 (`local-memory.sh`).
...
- **Features**: Long-term temporal, semantic, and entity-graph memory engine offloading embeddings, LLM reasoning, and reranking to `local-router` (port 21080) and PostgreSQL (`pgvector` + `pgroonga`). Worker control plane port configurable via `HINDSIGHT_API_WORKER_HTTP_PORT` (set to 0 to disable).
...
- **Signal Support**: Yes — Native channel integration communicating via the Go REST API wrapper (port 20889).
- **Coding Agent Support**: Yes — Natively supports **OpenCode** as a coding worker tool (`opencode_cli`).
- **Local LLM & Inference**: Supports local GGUF models via OpenAI-compatible endpoints served by `local-chat` (port 20080) or Ollama.
- **Embedding Options**: Local embeddings using the `local-embedding` server (port 20082) or Ollama, or OpenAI-compatible embedding APIs.
- **Reranking Support**: Native weighted hybrid search, or routes to external local-rerank service (`http://localhost:20086/v1/rerank`).
- **STT/TTS Support**: Natively routes voice uploads to local Whisper server (`local-speech-to-text` on port 20090) and local TTS via Qwen3-tts (`local-text-to-speech` on port 20095).
...
- **Signal Support**: Yes — Native integration communicating via the `signal-cli` HTTP daemon (port 20889).
...
- **Local LLM & Inference**: Supports local GGUF models via OpenAI-compatible endpoints served by `local-chat` (port 20080) or Ollama.
- **Embedding Options**: Local embeddings using the `local-embedding` server (port 20082) or Ollama, or remote/Ollama embeddings.
- **Reranking Support**: Native Reciprocal Rank Fusion (RRF) algorithm. No external reranker required.
- **STT/TTS Support**: Local STT via OpenAI-compatible transcription endpoint (`local-speech-to-text` on port 20090). No native TTS support.
...
- **Signal Support**: Yes — Native integration connecting to a local `signal-cli` HTTP daemon (port 20888/20889).
- **Coding Agent Support**: Yes — Supports Claude Code, Codex, and **OpenCode** via bundled skills.
- **Local LLM & Inference**: Supports local GGUF models via `local-chat` (port 20080) or Ollama.
- **Embedding Options**: Local embeddings via `local-embedding` (port 20082) or Ollama, or remote embedding providers (OpenAI, Cohere, Jina, Voyage AI).
- **Reranking Support**: Native reranking via auxiliary model slots and QMD hybrid retrieval engine, or routes to external reranker (`http://localhost:20086/v1/rerank`).
- **STT/TTS Support**: Local STT via local Whisper server (`local-speech-to-text` on port 20090). No native TTS support.
...
- **Signal Support**: Yes — Native integration via HTTP Server-Sent Events (SSE) (port 20888) with markdown-to-Signal formatting.
- **Coding Agent Support**: None (No OpenCode support).
- **Local LLM & Inference**: Routes to local GGUF models via `local-chat` (port 20080) or Ollama.
- **Embedding Options**: Local embeddings via `local-embedding` (port 20082) or Ollama, or remote embeddings.
- **Reranking Support**: No native reranking. Integrates with external reranker via custom MCP tools.
- **STT/TTS Support**: Local STT via local Whisper server (`local-speech-to-text` on port 20090). No native local TTS.
...
- **Signal Support**: Yes — Native channel integration interfacing with the Go REST API wrapper (port 20889), using `[[sidecar_channels]]` adapter `librefang.sidecar.adapters.signal`.
...
- **STT/TTS Support**: Local STT via local Whisper server (`local-speech-to-text` on port 20090) and local TTS via Qwen3-tts (`local-text-to-speech` on port 20095) supported via a patched package (`librefang-git` with `feature-local-stt-tts` patchset).
...
- **Local LLM & Inference**: Routes to local GGUF models via `local-chat` (port 20080) or Ollama.
- **Embedding Options**: Local embeddings via `local-embedding` (port 20082) or Ollama via API routing or MCP.
- **Reranking Support**: No native reranking. Reranking can be delegated via MCP to the local-inference reranker endpoint on port 20086.
- **STT/TTS Support**: Local STT by defining an ASR provider pointing to the local whisper-server on port 20090. No native TTS engine; requires an external MCP TTS tool.
...
- **Local LLM & Inference**: Routes to local GGUF models via OpenAI-compatible endpoints served by `local-chat` (port 20080) or Ollama.
- **Embedding Options**: Local embeddings via `local-embedding` (port 20082) or Ollama, or remote embeddings.
- **Reranking Support**: No native reranking. Reranking can be added via a custom skill or by configuring an MCP tool that calls the local-inference reranker endpoint on port 20086.
- **STT/TTS Support**: No native STT/TTS in the core daemon, but easily integrated via custom tools/skills calling `local-speech-to-text` (port 20090) and `local-text-to-speech` (port 20095).
- **Agent Client Protocol**: No native ACP support.
