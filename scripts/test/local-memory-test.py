...
async def run_test(base_url: str, bank_id: str):
    ...
    try:
        ...
        print(
            f"Error: Failed to connect to Hindsight server version endpoint: {e}",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        resp = await client.arecall(
            bank_id=bank_id, query="What is my primary programming language?"
        )
    ...
    try:
        resp = await client.areflect(
            bank_id=bank_id,
            query="Summarize what you know about my development context.",
        )
        text = resp.text or ""
        ...
        if not text:
            print(
                "Warning: Reflect succeeded but returned empty text.", file=sys.stderr
            )
    except Exception as e:
