...
# Import nanobot_mcp helpers if available
try:
    from omp_tools.nanobot_mcp import (
        get_pending_signal_messages,
        send_signal_message,
    )
except ImportError:
    try:
        from nanobot_mcp import (
            get_pending_signal_messages,
            send_signal_message,
        )
    except ImportError:
...
def extract_message_info(raw_msg: Dict[str, Any]) -> Optional[Tuple[str, str, str]]:
    ...
    if "envelope" in raw_msg and isinstance(raw_msg["envelope"], dict):
        ...
        sender = raw_msg.get("sender") or raw_msg.get("source") or raw_msg.get("recipient", "")
        text = raw_msg.get("message") or raw_msg.get("text") or raw_msg.get("body", "")
        msg_id = str(raw_msg.get("id") or raw_msg.get("uuid") or hash(f"{sender}:{text}"))
...
def query_hindsight_context(sender: str, prompt: str) -> str:
    ...
    if recalled_items:
        ...
        logger.info(f"Recalled {len(recalled_items)} memory context item(s) from Hindsight.")
        return context_str
...
def poke_omp_agent(prompt: str, context: str, sender: str) -> str:
    ...
    if isinstance(result, dict):
        return result.get("response") or result.get("text") or result.get("output") or json.dumps(result)
    elif isinstance(result, str):
        return result
    elif "response" in res:
        return str(res["response"])

    return str(result) if result is not None else "[OMP Gateway Error] Empty response from agent RPC."
...
def dispatch_signal_response(recipient: str, message: str) -> Dict[str, Any]:
    ...
    if send_signal_message is not None:
        try:
            ...
            if "error" not in res:
                logger.info(f"Dispatched Signal message to {recipient} via nanobot_mcp.")
                return res
            logger.warning(f"nanobot_mcp send failed: {res.get('error')}, falling back to direct HTTP.")
        except Exception as e:
    ...
    if "error" not in res:
        ...
        logger.error(f"Failed to dispatch Signal message to {recipient}: {res.get('error')}")
    return res
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Nanobot Signal Gateway RPC Bridge for OMP Agent.")
    parser.add_argument(
        "--poll-interval",
        type=float,
        default=3.0,
