#!/usr/bin/env python3
"""OMP Conveyor Ingestion Pipeline.
...
DEFAULT_INBOX_DIR = os.getenv(
    "CONVEYOR_INBOX", str(Path.home() / "Recordings" / "Inbox")
)
...
DEFAULT_STATE_FILE = str(Path.home() / ".omp" / "conveyor_processed_hashes.json")
...
logger = logging.getLogger("omp_conveyor")
...
def save_processed_hashes(state_file: Path, hashes: Set[str]) -> None:
    ...
    try:
        ...
        state_file.write_text(json.dumps(sorted(list(hashes)), indent=2), encoding="utf-8")
    except Exception as exc:
...
class ConveyorIngestor:
    """Conveyor Ingestion Pipeline manager."""
    ...
    def __init__(
        ...
        """Initialize the ConveyorIngestor instance.
        ...
        try:
            async with httpx.AsyncClient(timeout=120.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(
                        "STT service returned HTTP %d: %s", response.status_code, response.text
                    )
                    return ""
        except Exception as exc:
    ...
    async def retain_hindsight(
        ...
        tags = ["conveyor", "ingestion", item_hash, item_type]
        metadata = {
            "source": "conveyor",
            "filename": filename,
            "file_hash": item_hash,
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            try:
                ...
                if res.status_code in (200, 201, 202):
                    logger.info("Hindsight retention successful via /retain (HTTP %d).", res.status_code)
                    try:
                        data = res.json()
                    except Exception:
                        data = {"raw": res.text}
            ...
            try:
                        ...
                        "items": [
                            {
                                ...
                                "document_id": f"conveyor-{item_hash}",
                                "tags": tags,
                                "metadata": metadata,
                            }
                        ],
                ...
                try:
                    ...
                    data = {"raw": res.text}
                logger.info("Hindsight retention successful via /memories (HTTP %d).", res.status_code)
                return {"status": "success", "http_code": res.status_code, "data": data}
            except Exception as exc:
    ...
    async def process_file(self, file_path: Path) -> bool:
        ...
        if item_hash in self.processed_hashes:
            logger.debug("Skipping already processed file '%s' (hash: %s).", file_path.name, item_hash[:12])
            return False
        ...
        if sidecar_path:
            logger.info("Parsed sidecar '%s' for '%s': title='%s', type='%s'", sidecar_path.name, file_path.name, title, item_type)
        ...
        logger.info("Successfully ingested '%s' into Conveyor pipeline.", file_path.name)
        return True
    ...
    async def run_once(self) -> int:
        ...
        """
        logger.info("Executing Conveyor single scan pass (--once)...")
    ...
    async def run_watch(self, poll_interval: float = DEFAULT_POLL_INTERVAL_SECONDS) -> None:
        ...
        """
        logger.info(
            "Starting Conveyor Continuous Monitoring Loop (--watch) on %s (interval=%.1fs)...",
            self.inbox,
            poll_interval,
        )
        try:
            ...
            logger.info("Conveyor watch loop stopped.")
...
def parse_args(args: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="OMP Conveyor Ingestion Pipeline"
    )
    parser.add_argument(
        "--once",
        action="store_true",
        help="Execute a single scan pass over the inbox folder and exit",
    )
    parser.add_argument(
        "--watch",
        action="store_true",
        help="Run continuous monitoring loop over the inbox folder",
    )
    parser.add_argument(
        "--inbox",
...
async def main_async(cli_args: argparse.Namespace) -> int:
    ...
    # Default mode is watch if --once is not explicitly passed
...
def main() -> None:
    ...
    try:
        ...
        logger.info("Conveyor process interrupted by user. Exiting.")
        sys.exit(0)
