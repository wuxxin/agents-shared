...
| **Bunker Monitor** | TypeScript `bunker.ts` probing app health & test strategies | **Python Health Monitoring Daemon** (`omp_bunker_monitor.py`) probing ports 51080 (Local Router), 8888 (Hindsight), 50889 (Signal), 50090 (STT), and 50095 (TTS). |
...
| **System Diagnostics** | `Doctor.ts` checking system integrity points | Dedicated `omp doctor` diagnostic CLI script. |
...
### 2.2 Python Bunker Infrastructure Monitor (`omp_bunker_monitor.py`)

The **Bunker Monitor** tests infrastructure health across all local services (Local Router, Hindsight, Signal gateway, STT, TTS) and logs health metrics or alerts on state transitions.

```python
#!/usr/bin/env python3
"""OMP Bunker Infrastructure Monitor (Python).

Probes all local service endpoints, tracks health state transitions,
...
SERVICES: Dict[str, str] = {
    "Local Router": "http://localhost:51080/v1/models",
    "Hindsight Memory": "http://localhost:8888/v1/default/banks",
    "Nanobot Signal Gateway": "http://localhost:50889/health",
    "Speech-to-Text (STT)": "http://localhost:50090/v1/models",
    "Text-to-Speech (TTS)": "http://localhost:50095/v1/models",
}
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            ...
            if res.status_code in [200, 401, 404]:
                return name, True, f"HTTP {res.status_code}"
            return name, False, f"HTTP {res.status_code}"
        except Exception as err:
            return name, False, str(err)


async def run_bunker_health_check() -> Dict[str, Dict]:
    """Run parallel health probes across all configured infrastructure services."""
    tasks = [probe_service(name, url) for name, url in SERVICES.items()]
    results = await asyncio.gather(*tasks)

    health_report = {}
    for name, is_healthy, detail in results:
        status_str = "HEALTHY" if is_healthy else "DOWN/DEGRADED"
        health_report[name] = {"healthy": is_healthy, "detail": detail}
        logging.info(f"[Bunker Monitor] {name}: {status_str} ({detail})")
...
if __name__ == "__main__":
    ...
    asyncio.run(run_bunker_health_check())
```
