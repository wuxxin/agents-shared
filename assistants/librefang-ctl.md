...
3. **Strict Filesystem Isolation**
   ...
   - **Rationale**: The agent's persistent directories (`~/.local/sandbox/librefang`, `~/agent-shared`) are bind-mounted read-write, while the rest of the host filesystem is mounted read-only or hidden entirely.
   - **Custom Mounts**: Additional directories can be bind-mounted into the sandbox by configuring environment variables in `~/.config/systemd/user/librefang.env`:
