...
### Reranking, STT, and TTS Configuration

- **Reranking**: Reranking is not supported or configurable in this version of the LibreFang daemon.
- **Speech-to-Text (STT) / Text-to-Speech (TTS) Patches**: The patched AUR package (`librefang-git` with `feature-local-stt-tts` patchset) introduces custom base URL parameters to direct Whisper-transcription and TTS speech-generation requests to local split services (e.g. Kokoro, F5-TTS, or llama.cpp whisper-server).
  - **Local STT**: In `~/.librefang/config.toml`, configure the custom base URL under the `[media]` block:
    ```toml
    [media]
  ...
  - **Local TTS**: Configure the custom base URL under the `[tts.openai]` block:
    ```toml
    [tts]
    enabled = true
    provider = "openai"
