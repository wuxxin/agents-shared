#!/usr/bin/env bash
# local-embeddings.sh - Manage local llama-server systemd user service for Text Embeddings
#
# Usage: local-embeddings.sh <command> [args...]
#
# Manages a systemd user service (local-embeddings.service) that runs llama-server
...
load_env() {
    ...
    LE_PORT=50085
    LE_HOST=127.0.0.1
    LE_MODEL=/data/public/machine-learning/models/embedding/Qwen3-Embedding-0.6B-Q8_0.gguf
    LE_ALIAS=qwen3-embedding
    LE_N_CTX=8192
    LE_N_GPU_LAYERS=99
    LE_THREADS=4
    LE_EXTRA_ARGS="--flash-attn on"
...
get_shared_options() {
    ...
    echo "EnvironmentFile=-${home_spec}/.config/systemd/user/local-embeddings.env"
...
generate_service_file() {
    ...
    cat <<EOF
[Unit]
Description=Local Text Embedding Server (llama-server)
...
SyslogIdentifier=local-embeddings
...
generate_env_file() {
    cat <<'EOF'
# local-embeddings.env
# ---------------------------------------------------------------------------
# Configuration for the local-embeddings.service llama-server instance.
#
# Edit this file to switch models or tune runtime parameters.
# Reload with:  local-embeddings.sh restart
# ---------------------------------------------------------------------------

# Port to bind the server to (default: 50085)
LE_PORT=50085
...
cmd_test() {
    echo "Running local-embeddings validation tests..."
    load_env

    local host="${LE_HOST:-127.0.0.1}"
    local port="${LE_PORT:-50085}"
    local alias="${LE_ALIAS:-qwen3-embedding}"
