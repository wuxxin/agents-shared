...
4. **Strict Filesystem Isolation**
   ...
   - **Rationale**: Redirection of `HOME` to `~/.local/sandbox/nanoclaw` ensures that subprocesses do not write to the host user's real home. The persistent home, `~/agent-shared` are bind-mounted read-write, while other directories are read-only.
   - **Custom Mounts**: Additional directories can be bind-mounted into the sandbox by configuring environment variables in `~/.config/systemd/user/nanoclaw.env`:
